...
call :prompt_var SUPABASE_URL "Supabase URL"
if errorlevel 1 exit /b 1
call :prompt_var SUPABASE_SERVICE_ROLE_KEY "Supabase service role key"
if errorlevel 1 exit /b 1
set "SUPABASE_KEY=%SUPABASE_SERVICE_ROLE_KEY%"

echo.
echo === General configuration ===
call :ensure_var PORT "Port"
if errorlevel 1 exit /b 1
call :ensure_var GROQ_API_KEY "Groq API key"
if errorlevel 1 exit /b 1
call :ensure_var GROQ_MODEL "Groq model"
if errorlevel 1 exit /b 1
...
set "key=%~1"
set "label=%~2"
if "%label%"=="" set "label=%key%"
if not defined %key% (
    set /p "val=Enter %label%: "
    if "!val!"=="" (
